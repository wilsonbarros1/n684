      ...
      alunosArray.forEach((aluno) => {
        ...
        deleteButton.addEventListener("click", async () => {
          ...
          const shouldDelete = confirm(
            `Tem certeza que deseja deleta ${aluno.nome} do banco de dados?`
          );
          if (shouldDelete) {
            try {
...
const openModal = (el) => {
  ...
  nameLabel.setAttribute("for", "editIdade");
  const nameInput = document.createElement("input");
  nameInput.setAttribute("type", "text");
  nameInput.setAttribute("id", "editIdade");
...
const saveChanges = (id) => {
  ...
  const editedName = document.getElementById("editIdade").value;
